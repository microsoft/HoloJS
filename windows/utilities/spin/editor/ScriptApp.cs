...
namespace HoloJs.Spin
{
    public class ScriptApp
    {
        ...
        public static ScriptApp CreateNew(string appJson)
        {
            var returnValue = new ScriptApp();
        ...
        public static ScriptApp Open(string appJson)
        {
            var jsonContent = File.ReadAllText(appJson);
            var returnValue = JsonConvert.DeserializeObject<ScriptApp>(jsonContent);
            returnValue.AppPath = appJson;
            returnValue.HtmlAppPath = Path.Combine(Path.GetDirectoryName(appJson), Path.GetFileNameWithoutExtension(appJson) + ".html");
