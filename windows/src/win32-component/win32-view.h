...
class Win32HoloJsView : public IHoloJsView, public Win32HoloJsBaseView {
   public:
    Win32HoloJsView(HoloJs::ViewConfiguration viewConfiguration);
    virtual ~Win32HoloJsView();

    virtual HRESULT initialize(HoloJs::IHoloJsScriptHostInternal* host);
    virtual void setViewWindow(void* window) { throw new std::exception("Not supported"); }
    virtual void run();
    virtual void stop();
    ...
    virtual void setIcon(void* platformIcon) { m_icon = reinterpret_cast<HICON>(platformIcon); }
   ...
   private:
    ...
    std::shared_ptr<HoloJs::Win32::Win32MixedRealityContext> m_mixedRealityContext;
