...
void RenderingContext2D::fillText(
	std::wstring& text, float2& point, Color& color, int fontSize, std::wstring& fontFamily, std::wstring& fontWeight, std::wstring& fontStyle, int alignment)
{
...
double RenderingContext2D::measureText(std::wstring& text, int fontSize, std::wstring& fontFamily)
{
	...
	CanvasTextLayout ^ layout = ref new CanvasTextLayout(m_session, ref new Platform::String(text.c_str()), format, 0, 0);
...
float2 transformPoint(float3x2& mat, float2 pt) {
	return float2(
		mat.m11 * pt.x + mat.m21 * pt.y + mat.m31,
		mat.m12 * pt.x + mat.m22 * pt.y + mat.m32
	);
}
...
void RenderingContext2D::quadraticCurveTo(double cx, double cy, double x, double y)
{
	...
	if (!m_figurePresent) {
		...
		m_pathBuilder->AddQuadraticBezier(transformPoint(m_session->Transform, float2(cx, cy)), transformPoint(m_session->Transform, float2(x, y)));
	}
}
...
void RenderingContext2D::bezierCurveTo(double c1x, double c1y, double c2x, double c2y, double x, double y)
{
	...
	if (!m_figurePresent) {
		...
		m_pathBuilder->AddCubicBezier(transformPoint(m_session->Transform, float2(c1x, c1y)), transformPoint(m_session->Transform, float2(c2x, c2y)), transformPoint(m_session->Transform, float2(x, y)));
	}
}
...
void RenderingContext2D::arc(double cx, double cy, double r, double startAngle, double endAngle, bool counterClockwise) {
	...
	m_pathBuilder->AddArc(transformPoint(m_session->Transform, float2(cx, cy)), tr, tr, startAngle, endAngle - startAngle);
...
void RenderingContext2D::setLineStyle(float _lineWidth, Geometry::CanvasCapStyle capMode, Geometry::CanvasLineJoin joinMode) {
...
void RenderingContext2D::setGlobalOpacity(float opacity) {
...
void RenderingContext2D::getImageDataBGRFlipY(vector<byte>& bgrPixels)
{
	...
	for (int row_index = 0; row_index < m_height; row_index++) {
		for (int column_index = 0; column_index < m_width; column_index++) {
			...
			bgrPixels[destination_row_offset + column_index * dest_bpp + 0] = canvasPixelData[source_row_offset + column_index * m_bpp + 2] * s;
			bgrPixels[destination_row_offset + column_index * dest_bpp + 1] = canvasPixelData[source_row_offset + column_index * m_bpp + 1] * s;
			bgrPixels[destination_row_offset + column_index * dest_bpp + 2] = canvasPixelData[source_row_offset + column_index * m_bpp + 0] * s;
