...
void App::Load(Platform::String ^ entryPoint) { RecreateRenderer(); }
...
void App::OnWindowClosed(CoreWindow ^ sender, CoreWindowEventArgs ^ args) { mWindowClosed = true; }

void App::InitializeEGL(Windows::UI::Core::CoreWindow ^ window) { App::InitializeEGLInner(window); }
...
void App::InitializeEGLInner(Platform::Object ^ windowBasis)
{
    ...
    mEglSurface = eglCreateWindowSurface(
        mEglDisplay, config, reinterpret_cast<IInspectable*>(surfaceCreationProperties), surfaceAttributes);
